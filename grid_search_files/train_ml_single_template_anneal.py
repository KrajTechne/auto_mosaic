...
from prepare_single import SEQ_TARGET, CHAIN_MOTIF, MAX_OPTIMIZER_STEPS, DATA_DIR, PATH_INPUT_STRUCTURE, calculate_motif_rmsd, evaluate_optimized_structure, compute_composite_score, compute_harmonic_mean, generate_template_motif_annotation, extract_gemmi_chain, generate_template_target_annotation, extract_atom_array
...
class TargetFramePoseLoss(LossTerm):
    """RMSD of the motif after aligning the PREDICTED target interface onto the NATIVE
    target frame (6-DOF pose control). Unlike MotifRMSDLoss (Kabsch on the motif itself
    -> internal shape, blind to placement) and the distogram contact terms (constrain the
    trunk's distance BELIEF, not the coordinate placement), this pins WHERE the motif sits
    relative to the target: align pred target -> native target, apply that same transform
    to the pred motif, RMSD vs the native motif placement. Zero only when the motif is in
    the right position AND orientation on the target.

    Coordinate-derived (reads backbone_coordinates from the diffusion sample), so it needs
    25 sampling steps and is only meaningful with the motif TEMPLATE REMOVED -- with the
    template on, the pose is pinned and this term is trivially ~0. It is therefore added
    ONLY to the template-free loss.
    """
    motif_positions: Int[Array, "M"]          # binder-relative motif indices
    target_align_positions: Int[Array, "T"]   # target-relative indices defining the frame
    native_motif_ca: Float[Array, "M 3"]      # native motif CA (native complex frame)
    native_target_ca: Float[Array, "T 3"]     # native target CA, SAME frame & residue order

    name: str = "target_frame_pose"
    ...
    def __call__(
        ...
        binder_len = sequence.shape[0]
        # Predicted CA (index 1 is CA, matching MotifRMSDLoss / JointMotifRMSDLoss)
        pred_motif = output.backbone_coordinates[self.motif_positions, 1, :]                        # [M,3]
        pred_target = output.backbone_coordinates[self.target_align_positions + binder_len, 1, :]   # [T,3]

        # Kabsch: predicted target -> native target (same pure-JAX idiom as MotifRMSDLoss)
        pt_mu = pred_target.mean(axis=0)
        nt_mu = self.native_target_ca.mean(axis=0)
        P_c = pred_target - pt_mu
        T_c = self.native_target_ca - nt_mu
        ab = jnp.swapaxes(P_c, -1, -2) @ T_c
        u, s, vh = jnp.linalg.svd(ab, full_matrices=False)
        flip = jnp.linalg.det(u @ vh) < 0
        u_ = jnp.where(flip, -u[..., -1].T, u[..., -1].T).T
        ...
        # Apply the SAME transform to the predicted motif; compare to native motif placement.
        motif_in_native_frame = ((pred_motif - pt_mu) @ R) + nt_mu
        msd = jnp.mean(jnp.sum((motif_in_native_frame - self.native_motif_ca) ** 2, axis=-1))
        rmsd = jnp.sqrt(msd + 1e-8)
        return rmsd, {self.name: rmsd}
...
# Stepsize multiplier for the TEMPLATE-FREE phases (soft-off warm-up, sharp, hard). Removing
# the motif template makes the loss surface much stiffer -- the soft/sharp/hard stepsizes were
# tuned WITH the template's smoothing and overshoot once it's gone (the soft-off divergence /
# sharp-hard oscillation we diagnosed). This scales them down; the TEMPLATED soft-on stage is
# left untouched. Sweep in ~[0.2, 0.5] if the free phase still oscillates (watch the pose term:
# it should DESCEND through the switch, not climb).
FREE_STEPSIZE_SCALE = 0.3
...
def initialize_loss_function(loss_fn_weights: dict, motif_chain_order: list = ['D']):
    ...
    # Weight of the target-frame pose loss (coordinate-derived; added ONLY to the
    # template-free stages). Keep modest -- it is stiff, and too high destabilizes the
    # late stages (cf. the motif-RMSD blow-up). Let iptm/PAE do interface-quality work;
    # pose just pins placement once the motif template is gone.
    WEIGHT_POSE_LOSS_FUNCTION = loss_fn_weights.get('pose', 2.0)
    ...
    # soft-ON runs with the motif template (smooth surface) -> keep the tuned settings.
    soft_pssm_hyparams = {
    ...
    # soft-OFF warm-up runs template-free (stiffer surface): gentler stepsize + NO momentum
    # (the momentum was what turned the transition into a monotonic runaway).
    soft_off_pssm_hyparams = {
        'n_steps' : soft_pssm_hyparams['n_steps'],   # unused (n_off is derived in optimize_pssm)
        'stepsize' : FREE_STEPSIZE_SCALE * 0.2 * np.sqrt(SEQ_LEN_MUTABLE),
        'momentum' : 0.0,
        'scale'    : soft_pssm_hyparams['scale'],
        'logspace' : soft_pssm_hyparams['logspace'],   # False -> stays a simplex point
        'max_gradient_norm' : soft_pssm_hyparams['max_gradient_norm']
        }
    # sharp/hard are ALWAYS template-free in this design -> scale their stepsize down too.
    sharp_pssm_hyparams = {
        'n_steps' : 50,
        'stepsize' : FREE_STEPSIZE_SCALE * 0.5 * np.sqrt(SEQ_LEN_MUTABLE),
        'momentum' : 0.0,
        'scale'    : 1.25,
        'logspace' : True,
        'max_gradient_norm' : 1.00
    ...
    pssm_hyparams = {
        ...
        'soft_off' : soft_off_pssm_hyparams,
    ...
    # 6.3, base composite loss = complex structure terms + single-motif terms.
    # This is shared by BOTH the templated and template-free losses below.
    base_loss_fn = structure_prediction_loss + motif_first_loss
    ...
    # Also build the target-frame pose term here (added ONLY to the free loss below).
    pose = None
    if motif_contact_idx.shape[0] > 0:
        ...
        base_loss_fn = base_loss_fn + specific_contact + specific_pae + motif_scaffold_contact

        # --- Target-frame pose inputs -------------------------------------------------
        # Align on the interface target residues = the UNIQUE target partners in the
        # register (already the interface). target-relative, 0-based within SEQ_TARGET.
        target_align_positions = jnp.array(
            sorted(set(np.asarray(target_contact_idx).tolist())), dtype=jnp.int32)
        # Native target CA in SEQ_TARGET order (row t == SEQ_TARGET[t]); native motif CA
        # is already loaded as motif_ca_coords_first (same source PDB / same frame).
        aa_all = extract_atom_array(PATH_INPUT_STRUCTURE)
        ca_t = aa_all[(aa_all.atom_name == "CA") & (aa_all.chain_id == "C")]
        native_target_ca_all = ca_t.coord[np.argsort(ca_t.res_id)]
        assert native_target_ca_all.shape[0] == len(SEQ_TARGET), (
            f"target CA ({native_target_ca_all.shape[0]}) != len(SEQ_TARGET) "
            f"({len(SEQ_TARGET)}) -- target chain renumbered / missing residues; map explicitly")
        native_target_ca = native_target_ca_all[np.asarray(target_align_positions)]
        pose = TargetFramePoseLoss(
            motif_positions=motif_first_indices,
            target_align_positions=target_align_positions,
            native_motif_ca=jnp.array(motif_ca_coords_first),
            native_target_ca=jnp.array(native_target_ca),
        )

    # 6.4, two term-level losses. The FREE (template-off) loss additionally carries the
    # coordinate-derived pose term; the TEMPLATED (template-on) loss does not, because the
    # motif template pins the pose and the pose term would be trivially ~0 there.
    loss_fn_templated = base_loss_fn
    loss_fn_free = base_loss_fn if pose is None else (base_loss_fn + WEIGHT_POSE_LOSS_FUNCTION * pose)

    # 6.45, two feature sets. Both KEEP the target template (target stays well-modeled);
    # only the MOTIF template differs.
    #   templated: motif template ON  -> build the scaffold around the pinned motif
    #   free:      motif template OFF -> the scaffold/sequence must hold the pose itself
    #                                    (the same condition as template-free eval)
    feats_templated, _ = model_boltz.target_only_features(
    ...
    feats_free, _ = model_boltz.target_only_features(
        chains = [TargetChain(sequence = binder_seq, use_msa = False),
                  TargetChain(sequence = SEQ_TARGET, use_msa = True, template_chain = gemmi_target_chain)])

    # 6.5, wrap each: 25 sampling steps (pose/iptm/pae are coordinate-derived), then mask
    # gradients to the mutable "X" positions (SetPositions) and forbid Cys (NoCys). Two
    # feature sets -> two compilations (soft once, free once, reused for sharp/hard).
    def _wrap(loss_terms, feats):
        ms = model_boltz.build_multisample_loss(
            loss = loss_terms, features = feats,
            sampling_steps = 25, recycling_steps = 1, num_samples = 4)
        return NoCys(SetPositions.from_sequence(wildtype = binder_seq, loss = ms))

    masked_cys_loss_templated = _wrap(loss_fn_templated, feats_templated)  # template ON,  no pose
    masked_cys_loss_free      = _wrap(loss_fn_free,      feats_free)       # template OFF, + pose

    return masked_cys_loss_templated, masked_cys_loss_free, pssm_hyparams, model_boltz

#-------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Optimization: Define Initial PSSM and Optimize in 3 Stages: Soft -> Sharp -> Discrete
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------
SOFT_TEMPLATE_ON_FRAC = 0.75   # fraction of the soft stage run with the motif template ON

def optimize_pssm(loss_soft, loss_free, pssm_hyparams, seed: int):
    """Staged optimization with TEMPLATE ANNEALING.

    The soft stage is split: the first SOFT_TEMPLATE_ON_FRAC of soft steps run against
    `loss_soft` (motif template ON, no pose term) to build the scaffold around the pinned
    motif; the remaining soft steps run against `loss_free` (motif template OFF, pose term
    ON) while the PSSM is still soft -- a gentle template-free warm-up before sharpening.
    SHARP and HARD then continue on `loss_free`, so the sequence is sharpened to a discrete
    design under the SAME (template-free) condition as evaluation: the scaffold must hold
    the pose without the crutch. The TARGET template is kept throughout (baked into both
    losses); only the MOTIF is un-pinned.

    `loss_soft` and `loss_free` are the two wrapped losses returned by
    initialize_loss_function. Key handling is unchanged from the original (same fixed key
    passed to every stage).
    """
    # 1. Create Initial PSSM
    num_mutable_residues = pssm_hyparams['num_mutable_residues']
    # 2. Initialize key from the fixed seed (same seed reused across grid configs == common
    # random numbers, so differences in downstream metrics reflect weights, not the init)
    ...
    soft = pssm_hyparams['soft']
    n_soft = soft['n_steps']
    n_on = int(round(SOFT_TEMPLATE_ON_FRAC * n_soft))   # template ON  (build the scaffold)
    n_off = n_soft - n_on                               # template OFF (settle pose, no crutch)

    # 2a. SOFT / template ON -> place the motif, rough in the scaffold (loss_soft)
    print(f"[optimize_pssm] SOFT | motif template ON  | loss_soft | {n_on} steps", flush=True)
    _, pssm = simplex_APGM(loss_function= loss_soft,
                           x=jax.nn.softmax(pssm_initial),
                           n_steps= n_on,
                           stepsize= soft['stepsize'],
                           momentum= soft['momentum'],
                           scale= soft['scale'],
                           logspace= soft['logspace'],
                           max_gradient_norm= soft['max_gradient_norm'],
                           key = key
                       )

    # 2b. SOFT / template OFF -> carry the (still-soft) simplex PSSM onto loss_free.
    # Gentler free-phase settings (soft_off: stepsize x FREE_STEPSIZE_SCALE, momentum 0) on the
    # stiffer template-free surface. Still logspace=False, so x stays a simplex point (no log).
    if n_off > 0:
        soft_off = pssm_hyparams['soft_off']
        print("=" * 78, flush=True)
        print(f"[optimize_pssm] >>> MOTIF TEMPLATE OFF <<< | loss_free (+pose) | soft warm-up | {n_off} steps", flush=True)
        print(f"    free-phase stepsize x{FREE_STEPSIZE_SCALE} ({soft_off['stepsize']:.3f}), momentum {soft_off['momentum']}", flush=True)
        print("    in-loop iptm/motif_rmsd now become HONEST -- watch for the drop from the", flush=True)
        print("    template-saturated values; that drop is the true interface quality.", flush=True)
        print("=" * 78, flush=True)
        _, pssm = simplex_APGM(loss_function= loss_free,
                               x=pssm,
                               n_steps= n_off,
                               stepsize= soft_off['stepsize'],
                               momentum= soft_off['momentum'],
                               scale= soft_off['scale'],
                               logspace= soft_off['logspace'],
                               max_gradient_norm= soft_off['max_gradient_norm'],
                               key = key
                           )

    # 3. SHARP / template OFF (loss_free): sharpen toward a discrete sequence
    print(f"[optimize_pssm] SHARP | motif template OFF | loss_free (+pose) | {pssm_hyparams['sharp']['n_steps']} steps", flush=True)
    pssm, _ = simplex_APGM(loss_function= loss_free,
                       x=jnp.log(pssm + 1e-5),
    ...
    # 4. HARD / template OFF (loss_free): tighten toward a (near) one-hot sequence
    print(f"[optimize_pssm] HARD | motif template OFF | loss_free (+pose) | {pssm_hyparams['hard']['n_steps']} steps", flush=True)
    pssm, _ = simplex_APGM(loss_function= loss_free,
                       x=jnp.log(pssm + 1e-5),
    ...
    # 5. Add Cysteine back into the PSSM (NoCys/SetPositions wrappers are identical on both
    #    losses, so either unwraps correctly; use the free loss, the last one optimized).
    pssm_with_cysteine = loss_free.sequence(pssm)
    # 6. Add Fixed Residues back into PSSM
    pssm_with_fixed_residues = loss_free.loss.sequence(pssm_with_cysteine)
...
def evaluate_optimized_pssm(pssm_optimized, model_boltz, CHAIN_MOTIF, DATA_DIR):
    ...
    rmsd_d_esmfold, iptm_esmfold, plddt_esmfold = evaluate_optimized_structure(struc_model= model_esm2,seq_pssm= pssm_optimized,motif_id_pos= CHAIN_MOTIF, design_iteration = design_iteration, model_name= "ESMFold2")
...
def main():
    ...
    with mlflow.start_run(tags = run_tags):
        ...
        # 1. Initialize loss functions (templated + template-free), optimizer hyparams, Boltz model
        loss_soft, loss_free, pssm_hyparams, model_boltz = initialize_loss_function(loss_fn_weights= loss_fn_weights)
        # 2. Optimize the pssm with template annealing (soft ON->OFF, sharp+hard OFF)
        pssm_optimized = optimize_pssm(loss_soft= loss_soft, loss_free= loss_free, pssm_hyparams= pssm_hyparams, seed= args.seed)
